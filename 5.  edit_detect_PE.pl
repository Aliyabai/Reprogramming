...
close IN;
foreach my $tumor (sort keys %hash){
    unless (-d "$outdir/$tumor") {`mkdir $outdir/$tumor`;}
    my $i = 0;
    foreach my $sample (sort keys %{$hash{$tumor}}){
        my $bam = $hash{$tumor}{$sample};
        unless (-d "$outdir/$tumor/$sample") {`mkdir -p $outdir/$tumor/$sample`;}
        my $output = "$outdir/$tumor/$sample";
        unless (-d "$outdir/$tumor/singleprocessrna") {`mkdir -p $outdir/$tumor/singleprocessrna`;}
        $i += 1;
        open OUT, ">$outdir/$tumor/singleprocessrna/H49.removesnp.$i.sh" or die $!;
...
ard the non-unique mapping reads 筛选最小匹配率大于20包含mismatch率的unique reads
        ...
        #
        print OUT "perl $bin/end.strand.mism.poly.rep.filt.pl --in $output/binomFilter.filt.fre.strand.txt --out $output/binomFilter.filt.fre.strand.e
nd.simplerepeat.homopolymer.txt --simpleRepeat $simpleRepeat --reference $chr --endfre 0.9 --endp 0.05 --strandfre 0.9 --strandp 0.005 --homopolymer 5
;echo 4 complete\n";
        #
...
output/binomFilter.filt.fre.strand.end.simplerepeat.homopolymer.dbsnp.txt;echo 5 complete\n";
        #
        print OUT "perl $bin/pos_filter.pl --input $snpdir/1000g.YH.Mongolian.dai.Dutch.snp.txt.gz --loci $output/binomFilter.filt.fre.strand.end.simp
lerepeat.homopolymer.dbsnp.txt --out $output/1000g.YH.Mongolian.dai.Dutch.exchange.txt;echo 6 complete\n";
        #
        print OUT "perl $bin/pos_filter.pl --input $output/binomFilter.filt.fre.strand.end.simplerepeat.homopolymer.dbsnp.txt -loci $output/1000g.YH.M
ongolian.dai.Dutch.exchange.txt --filt 2 --out $output/binomFilter.filt.fre.strand.end.simplerepeat.homopolymer.dbsnp.snp.txt;echo complete 7\n";
        ...
        print OUT "perl $bin/extract_reads.pl $bam $output/bwa/mutation.read1.fq $output/bwa/mutation.read2.fq $output/bwa/mutation.read.fq $output/bi
nomFilter.filt.fre.strand.end.simplerepeat.homopolymer.dbsnp.snp.txt $output/readsvnum.txt;echo complete 9\n";
        print OUT "$bwa aln -o 1 -l 31 -k 2 -t 4 -L -i 15 $database $output/bwa/mutation.read1.fq > $output/bwa/mutation.read1.sai;echo complete 10\n"
;
        print OUT "$bwa aln -o 1 -l 31 -k 2 -t 4 -L -i 15 $database $output/bwa/mutation.read2.fq > $output/bwa/mutation.read2.sai;echo complete 11\n"
;
        
        print OUT "$bwa sampe -a 170*1.3 $database $output/bwa/mutation.read1.sai $output/bwa/mutation.read2.sai $output/bwa/mutation.read1.fq $output
/bwa/mutation.read2.fq |/ifs1/ST_SYSBIO/USER/liudb2/bin/TumorCare/bin/samtools view -b -S - -t $database.fai > $output/bwa/mutation.readP.bam;echo com
plete 12\n";
        ###
...
tools view -b -S -t $database.fai - > $output/bwa/mutation.readS.bam;echo complete 14\n";
        ###
        print OUT "perl $bin/bwa.fre.filt.pl $output/bwa/mutation.readP.bam $output/bwa/mutation.readS.bam $output/binomFilter.filt.fre.strand.end.sim
plerepeat.homopolymer.dbsnp.snp.txt $refgene $output/readsvnum.txt $output/binomFilter.filt.fre.strand.end.simplerepeat.homopolymer.dbsnp.snp.bwa.txt;
echo complete 15\n";
        ###
